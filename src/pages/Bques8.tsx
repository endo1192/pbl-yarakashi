...
export default function Home({ count, arrayn, answer, selected }: Bques1Props) {
  ...
  return ( 
    <div className="answer">
      ...
      <p>{Canswer}</p>
      <li>
      <Link href={{ pathname: '/Bans8', query: { num: 1, arrayn: JSON.stringify(Carray), count: Cnumber, answer: JSON.stringify(Canswer), selected: Selected, } }}>1:口コミなどの情報をよく見てから、購入するかを考える。</Link></li><br />
      <li><Link href={{ pathname: '/Bans8', query: { num: 2, arrayn: JSON.stringify(Carray), count: Cnumber, answer: JSON.stringify(Canswer), selected: Selected, } }}>2:安いのですぐに購入する。</Link></li><br />
      <li><Link href={{ pathname: '/Bans8', query: { num: 3, arrayn: JSON.stringify(Carray), count: Cnumber, answer: JSON.stringify(Canswer), selected: Selected, } }}>3:信用できるサイトだから大丈夫だろうと思い、購入する </Link></li><br />
